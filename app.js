...
// Changelog:
//   v2.5.0 (Sep 24, 2026): Accessibility, filtering & BT-purple rebrand (ported
//           from the a11y-ux prototype; verified Lighthouse Accessibility 100 in
//           both light and dark themes).
//           * Interaction consistency + keyboard operability: kanban cards, list
//             rows and sortable headers are focusable and Enter/Space-activatable
//             (one shared role=button handler), matching the dashboard stat cards.
//             Dashboard hover/focus previews a filter with a SINGLE border;
//             click/Enter LOCKS it with a DOUBLE border — same for mouse and
//             keyboard. The lock border is drawn on the container (kanban column /
//             list-view box), not each card. Dimmed cards are removed from the tab
//             order + aria-hidden while a filter is active.
//           * New toolbar Contributors dropdown filter; renamed the Category/
//             Priority dropdown placeholders to "Categories"/"Priority".
//           * New dashboard "Contributors" stat (distinct submitters); dashboard
//             counts now reflect the active toolbar filters.
//           * Context-aware "Clear filters (N)" button (shown only when a filter
//             is active) + Esc clears all filters. Filter result count announced.
//           * Header reworked: single "Arch. Middleware Team Innovation Tracker"
//             label with the version badge + last-updated by the title.
//           * a11y hardening: modals are role=dialog aria-modal with focus trap +
//             restore; #sr-live polite region; prefers-reduced-motion; base tap
//             targets >=24px; WCAG-AA contrast (dark --text-light, tokenised kanban
//             headers, stat-number colours, on-primary ink).
//           * Branding: BT-purple palette (--primary #5514b4 light / #b39dff dark)
//             replacing the old indigo, across UI + theme-color + icons.
//           Bug fixes: dashboard filter no longer drops when a modal opens; the
//           first stat-card click now locks the correct filter (was showing all).
...
// localStorage keys.
var LS = { data: 'ib_data', user: 'ib_user', theme: 'ib_theme', tourDone: 'ib_tour_done' };
...
// BOOTSTRAP ADMIN SEED (case-insensitive). NOT the ongoing source of truth for
// who is an admin — roles live in the DB (see isAdmin/registerUser). This only
// mints the FIRST admin so there's someone able to promote others; once a role
// is stored, the DB wins (a seeded name can even be demoted).
...
function showAppVersion() {
  ...
  if (el) { el.textContent = 'v' + APP_VERSION; el.title = 'Idea Board v' + APP_VERSION; }
}
  ...
  if (state.firebaseReady && state.onlineUsers) {
    ...
    if (!userIds.length) {
      ...
      userIds.forEach(function(uid) {
        ...
        html += '<div style="width:28px;height:28px;border-radius:50%;background:var(--primary);color:var(--on-primary);display:flex;align-items:center;justify-content:center;font-size:.7rem;font-weight:700">' + initial + '</div>';
...
function renderCard(idea) {
  ...
  // Card is a keyboard-operable button: role/tabindex/aria-label + the shared
  // Enter/Space handler open the detail, matching the stat cards.
...
var _tourSteps = [
  ...
  { target: '.search-box', title: 'Search & Filter', text: 'Search by title, description, or submitter. Use the dropdowns to filter by category, priority or contributor, and Clear filters (or Esc) to reset.' },
