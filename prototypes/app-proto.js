...
// role, the DB wins (a seeded name can even be demoted). On the gated host the
// first admin can instead be seeded by UPN in auth-config.js — see BOOTSTRAP_ADMIN_UPNS.
var ADMIN_NAMES = ['isaac gera'];

// Optional: seed the first admin by verified Entra UPN/email (gated host).
// Lets us drop the name-based seed entirely on BT without a lockout risk.
var BOOTSTRAP_ADMIN_UPNS = ['isaac.2.gera@bt.com'];
...
function loadUser() {
  ...
  if (identity && identity.name) {
    // Gated host: stable per-user id from the verified token (oid/sub) so the
    // same person ALWAYS maps to one record — no duplicates across sessions or
    // devices. Fall back to a UPN-derived id, then a name-derived id.
    var stableId;
    if (identity.oid) stableId = 'entra_' + identity.oid;
    else if (identity.username) stableId = 'entra_' + slugifyName(identity.username);
    else stableId = localIdForName(identity.name);
    state.currentUser = {
      id: stableId,
      name: identity.name,
...
// Normalise a name/UPN into a stable slug for deterministic ids.
function slugifyName(s) {
  return String(s || '').trim().toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '');
}

// Deterministic id for the local/ungated path: the same typed name always
// resolves to the same record, so re-entering your name never spawns a
// duplicate user. (Genuinely different names stay separate, by design.)
...
function registerUser() {
  ...
  ref.once('value').then(function (snap) {
    ...
    // If the read fails, fall back to a safe write that still can't downgrade a
    // promoted admin blindly: seed by bootstrap only.
...
// local id so a person who later signs in / enters this exact name inherits the
// pre-assigned role. Useful for testing and pre-granting a role.
// NOTE: on the gated (Entra) host a real user's id is entra_<oid>, which can't
...
function demoteUser(userId) {
  ...
  // of admin functions with no way back (roles are DB-driven now, no hardcoded
  // rescue). Count admins by distinct name so merged records don't over-count.
