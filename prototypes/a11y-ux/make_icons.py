"""
Idea Board a11y PROTOTYPE — PWA icon generator (pure standard library).

Same generator as the live app's make_icons.py, but with the BT-PURPLE brand
palette used by the a11y-ux prototype (was indigo). Writes valid PNGs using
only zlib + struct — no Pillow, no network (works behind the corporate proxy).

Renders the brand mark: a BT-purple rounded-square tile with a white outline
lightbulb glyph and an amber filament dot, matching the header brand-icon.

Outputs (in this prototype folder):
  ...
  screenshot-wide.png   1280x720 branded placeholder (manifest, desktop)
  screenshot-narrow.png 720x1280 branded placeholder (manifest, mobile)

Run:  python make_icons.py   (from this prototypes/a11y-ux folder)
"""
import struct
import zlib
import math

# BT-purple brand palette (matches ideaboard.html :root --primary family).
PRIMARY = (85, 20, 180)        # #5514b4 BT purple
PRIMARY_LIGHT = (126, 63, 242)  # #7e3ff2
ACCENT = (245, 158, 11)        # #f59e0b amber (bulb filament glow)
WHITE = (255, 255, 255)
...
def draw_tile(px, size, inset):
    """BT-purple diagonal-gradient rounded tile filling (size - 2*inset)."""
    left = inset
    top = inset
    right = size - inset
    bottom = size - inset
...
def draw_bulb(px, size, inset):
    """Line-drawn (outline) lightbulb glyph: a stroked white globe over a small
    stroked screw-base, with an amber filament dot. Stroke scales with size."""
    area = size - 2 * inset
    cx = size / 2.0
    stroke = max(2.0, area * 0.055)
    globe_r = area * 0.19
...
def write_png(path, px, size=None):
    """Write an RGBA PNG; width/height derived from the pixel array."""
    height = len(px)
    width = len(px[0])
...
def make_screenshot_sized(path, w, h):
    """Branded placeholder screenshot for the manifest install UI."""
    px = [[(248, 250, 252, 255) for _ in range(w)] for _ in range(h)]
    ...
    for i in range(kcols):
        lx = m + i * (kw + gap)
        rrect(lx, ky, lx + kw, ky + kh, 10, (241, 245, 249))
        for c in range(2):
